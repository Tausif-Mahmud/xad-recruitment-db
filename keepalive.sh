...
if [ ! -f "$YAML_FILE" ] || [ ! -s "$YAML_FILE" ] || ! grep -q "name: Keep Streamlit Apps Alive" "$YAML_FILE"; then
...
sed -i "s/cron: .*/cron: '$NEW_CRON'/" "$YAML_FILE"

add_app() {
    clear
    echo "Adding a new application..."
    read -p "Enter the URL of the new Streamlit app: " NEW_URL
